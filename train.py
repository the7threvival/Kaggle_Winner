...
def transform_train(image, mask, label):
    ...
    if 1:
        if random.random() < 0.5:
            ...
            # Create a new id when flipped l->r
...
def train(debug, freeze=False, fold_index=1, model_name='seresnext50',min_num_class=10, checkpoint_start=0, lr=3e-4, batch_size=36, num_classes=10008):
    import ipdb
    ...
    # TODO: pass the number of total classes if including new_whale
    ...
    # https://github.com/pytorch/pytorch/issues/1356
    # ended up fixing by disabling something in cv at the top of the file
...
if __name__ == '__main__':
    ...
    if 1:
        ...
        try:
          ...
          lr = float(sys.argv[3])
          freeze = bool(int(sys.argv[4]))
        except:
          print("Input: ", sys.argv)
          print("Usage: {} fold_index checkpoint_start lr min_num_class freeze".format(sys.argv[0]))
          print("Default: new_flukes 0 3e-4 0 0")
          print("freeze must be either 1(True) or 0(False)")
          exit(1)
          # fold_index = "flukes10"
          # checkpoint_start = 0
          # lr = 3e-4
        ...
        print("Some params:")
        print("Fold_index: {}, Check_start: {}, Learning rate: {}, Min # of Occ.: {}, Freeze: {}".format(fold_index, checkpoint_start, lr, min_num_class, freeze))
        print("!!! Sanity check !!!")
        print("Input: ", sys.argv)
        print()
